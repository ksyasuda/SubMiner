...
**Choosing the screenshot.** With still images enabled, the review also shows a screenshot preview. Drag its separate slider to choose an image without changing the audio, or use the Frame buttons to move to the previous or next decoded video frame. Earlier and Later reveal more time for both sliders. The image follows the audio midpoint until you pick a frame; after that it stays fixed while you trim or extend the audio. Use midpoint restores that automatic behavior. Reset in the audio controls resets only the audio selection. Keyboard users can focus the screenshot slider and use the arrow keys, Home, and End.

The picker works with local video and seekable remote streams, including Jellyfin, and reuses the downloaded review window when the audio and video share a source. Previewing does not seek the main player. A manually selected image must finish loading successfully before you can confirm; if it cannot load, choose another time or use the midpoint. Image selection is hidden when image generation is disabled or animated AVIF is selected. Animated images continue to use the reviewed clip range.
