...
test('requestYomitanTermFrequencies uses configured Yomitan sort frequency dictionary', async () => {
  ...
  const deps = createDeps(async (script) => {
        ...
        profiles: [
          {
            options: {
              scanning: { length: 40 },
              general: {
                sortFrequencyDictionary: 'JPDBv2',
              },
              dictionaries: [
                { name: 'Jitendex', enabled: true, id: 0 },
                { name: 'JPDBv2', enabled: true, id: 10 },
                { name: 'Jiten', enabled: true, id: 11 },
                { name: 'CC100', enabled: true, id: 12 },
              ],
            },
          },
        ],
    ...
    return [
      ...
      },
      {
        ...
        dictionary: 'CC100',
        frequency: 61732,
        displayValue: '23 61732',
        displayValueParsed: true,
      },
    ];
  });

  const result = await requestYomitanTermFrequencies([{ term: '言う', reading: 'いう' }], deps, {
    error: () => undefined,
  });

  assert.equal(frequencyScripts.length, 1);
  assert.match(frequencyScripts[0] ?? '', /dictionaries:\s*\["JPDBv2"\]/);
  assert.deepEqual(result.map(({ dictionary }) => dictionary), ['JPDBv2']);
  assert.equal(result[0]?.dictionaryPriority, 0);
  assert.equal(result[0]?.frequency, 27860);
});
...
test('requestYomitanScanTokens maps top-result Yomitan tags onto token pos1', async () => {
  const deps = createDeps(async (script) => {
    if (script.includes('optionsGetFull')) {
      return {
        profileCurrent: 0,
              ...
              headwords: [
                {
                  term: 'クソッ',
                  reading: 'くそっ',
                  tags: [{ name: 'interjection' }],
                  sources: [{ originalText: 'クソッ', isPrimary: true, matchType: 'exact' }],
                },
              ],
              definitions: [],
            },
  ...
  const result = await requestYomitanScanTokens('クソッ', deps, {
    error: () => undefined,
  });

  assert.equal(result?.[0]?.pos1, '感動詞');
});

test('requestYomitanScanTokens falls back to second or third result when earlier Yomitan entries have no POS tags', async () => {
  const deps = createDeps(async (script) => {
    if (script.includes('optionsGetFull')) {
      return {
        profileCurrent: 0,
              ...
              headwords: [
                {
                  ...
                  reading: 'お',
                  tags: [],
                  sources: [{ originalText: 'お', isPrimary: true, matchType: 'exact' }],
                },
              ],
              definitions: [],
            },
  ...
  const result = await requestYomitanScanTokens('お', deps, {
    error: () => undefined,
  });

  assert.equal(result?.[0]?.pos1, '接頭詞');
});

test('requestYomitanScanTokens extracts normalized POS tags from the first tagged result within top 3 entries', async () => {
  let scannerScript = '';
  const deps = createDeps(async (script) => {
  ...
  await requestYomitanScanTokens('クソッ', deps, { error: () => undefined });
    ...
    return {
      ...
      dictionaryEntries: [
        ...
        },
        {
          ...
          definitions: [
            {
              tags: [{ category: 'partOfSpeech', name: 'interjection' }],
            },
          ],
        },
      ],
  ...
  assert.equal((result as Array<{ pos1?: string }>)[0]?.pos1, '感動詞');
  assert.equal((result as Array<{ pos2?: string }>)[0]?.pos2, '');
});
