...
test('buildProtectedSupportAssetsCommand installs both theme and plugin assets', () => {
  ...
  assert.match(command, /sudo cp "\$tmp\/assets\/themes\/subminer\.rasi" '\/usr\/local\/share\/SubMiner'\\''s data'\/themes\/subminer\.rasi/);
  assert.match(command, /sudo rm -rf '\/usr\/local\/share\/SubMiner'\\''s data'\/plugin\/subminer/);
  assert.match(command, /sudo mkdir -p '\/usr\/local\/share\/SubMiner'\\''s data'\/plugin/);
  assert.match(command, /sudo cp -R "\$tmp\/plugin\/subminer" '\/usr\/local\/share\/SubMiner'\\''s data'\/plugin\/subminer/);
});
...
test('updateSupportAssetsFromRelease installs missing theme and plugin into an existing managed root', async () => {
  const xdgDataHome = fs.mkdtempSync(path.join(os.tmpdir(), 'subminer-xdg-data-'));
