...
function getBestLatestVersion(currentVersion, appUpdate, release) {
    ...
    const candidates = [appUpdate.version, releaseVersion].filter((value) => typeof value === 'string' && value.length > 0);
    const latest = candidates.reduce((best, candidate) => ((0, release_assets_1.compareSemverLike)(candidate, best) > 0 ? candidate : best), currentVersion);
    return {
        available: appUpdate.available || (0, release_assets_1.compareSemverLike)(latest, currentVersion) > 0,
        version: latest,
    };
}
...
function createUpdateService(deps) {
    ...
    async function runCheck(request) {
        ...
        try {
            ...
            const shouldFetchReleaseMetadata = deps.shouldFetchReleaseMetadata?.({ request, channel, appUpdate }) ?? true;
            const release = shouldFetchReleaseMetadata
                ? await deps.fetchLatestStableRelease(channel).catch((error) => {
                    deps.log(`GitHub release update check failed: ${error.message}`);
                    return null;
                })
                : null;
...
function createFileUpdateStateStore(statePath) {
    return {
        ...
        async writeState(state) {
            await node_fs_1.default.promises.mkdir(node_path_1.default.dirname(statePath), { recursive: true });
            await node_fs_1.default.promises.writeFile(statePath, `${JSON.stringify(state, null, 2)}\n`, 'utf8');
        },
    };
}
